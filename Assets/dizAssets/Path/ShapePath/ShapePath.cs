...
public class ShapePath {
	...
	*/
		...
		for ( int i = 0; i < cmd_len; i++ )
		{
			...
			switch( c )
			{
				...
				case PathType.__LINE_TO:
					...
					PathLine pline = new PathLine( px, py, pz, x, y, z );
					//__path[path_i] = pline;
					__path.Add( pline );
				...
				case PathType.__CURVE_TO:
					...
					QBezier pbezier =  new QBezier( px, py, pz, b__coordinates[ci], b__coordinates[ci + 1], b__coordinates[ci + 2], x, y, z );
	...
	public void curveTo ( float cx, float cy, float cz, float x, float y, float z ){
	...
	public void curveTo ( Vector3 cPosition, Vector3 position ){
		curveTo( cPosition.x, cPosition.y, cPosition.z, position.x, position.y, position.z );
	}
	public void lineTo ( Vector3 position ){
		lineTo( position.x, position.y, position.z );
	}
